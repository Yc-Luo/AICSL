    ...
    <div className="min-h-screen bg-gray-50 flex font-sans text-slate-900 overflow-hidden">
      ...
      `}>
          ...
          <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center' : ''}`}>
            ...
            {!isCollapsed && (
              <div className="animate-fadeIn">
                <span className="block font-bold text-slate-800 text-lg">AICSL</span>
        ...
        <nav className={`flex-1 ${isCollapsed ? 'px-2' : 'px-4'} py-8 space-y-2 overflow-y-auto`}>
          ...
          />
          <SidebarLink
            to={ROUTES.TEACHER.RAG_KNOWLEDGE_BASE}
            icon={Database}
            label="RAG 知识库"
            active={isActive(ROUTES.TEACHER.RAG_KNOWLEDGE_BASE)}
            isCollapsed={isCollapsed}
          />
          <SidebarLink
            to={ROUTES.TEACHER.AGENT_CONFIG}
            icon={Brain}
            label="智能体配置"
            active={isActive(ROUTES.TEACHER.AGENT_CONFIG)}
            isCollapsed={isCollapsed}
          />
          <SidebarLink
