...
import { ChatMessage } from '../../../../modules/chat/ChatPersistence' // Use shared type
...
export default function ChatPanel({ projectId }: ChatPanelProps) {
      ...
      try {
        ...
        if (response.data && response.data.messages && isMounted) {
          ...
          const history: ChatMessage[] = response.data.messages.map((m: any) => ({
            ...
            mentions: m.mentions || [],
            timestamp: m.created_at, // Map created_at to timestamp
            isPending: false
          ...
          setMessages(currentMessages => {
            ...
            // 1. Add history first
            history.forEach((m: ChatMessage) => msgMap.set(m.id, m));

            // 2. Add current messages (preserving pending state or optimistic updates)
            currentMessages.forEach((m: ChatMessage) => msgMap.set(m.id, m));
  ...
  const AI_AGENTS = [
    { id: 'ai-coord', name: '学习协调者', description: '引导讨论方向' },
    { id: 'ai-sum', name: '内容总结者', description: '整理核心结论' },
    { id: 'ai-crit', name: '批判性思维', description: '提供深度反馈' },
    { id: 'ai-res', name: '资料研究员', description: '提供背景知识' }
  ]
  ...
  useEffect(() => {
    const onTyping = (data: any) => {
      ...
      setTypingUsers(prev => {
        ...
        next.add(data.username || 'Someone')
        return next
      })
      messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' })
    }
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {messages.map((msg: ChatMessage, index: number) => {
          ...
          return (
            ...
            >
              <div
                className={`max-w-[85%] lg:max-w-[75%] px-3 py-2 rounded-xl transition-all ${msg.user_id === user?.id
