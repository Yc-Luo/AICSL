    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isEdit && project) {
                ...
                const newProj = await projectService.createProject({ ...formData, owner_id: '' } as any) // owner_id handled by backend usually
                // Add members if any
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
                <DialogHeader>
                    <DialogTitle>{isEdit ? '修改项目' : '创建新项目'}</DialogTitle>
                    <DialogDescription className="text-sm text-slate-500">
                        {isEdit ? '编辑项目基本信息、成员配置以及初始学习资源。' : '建立一个新的协作空间，配置基础信息并添加初始成员。'}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-6 py-4">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <label className="text-sm font-medium text-slate-700">项目成员 ({members.length})</label>
                        </div>
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-100">
                        ...
                        {!isEdit ? (
                            <p className="text-xs text-slate-400 italic">创建项目后即可上传初始资源</p>
                        ) : resources.length > 0 ? (
