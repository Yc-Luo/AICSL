...
async def generate_presigned_url(
    ...
    # Check storage quota
    current_usage = await get_project_storage_usage(project_id)
    max_storage = settings.MAX_PROJECT_STORAGE  # Default: 5GB
    
    if current_usage >= max_storage:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail=f"Project storage quota exceeded. Used: {current_usage / (1024**3):.2f}GB, Limit: {max_storage / (1024**3):.2f}GB",
        )
