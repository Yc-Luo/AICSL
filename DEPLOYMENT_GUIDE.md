...
本指南提供完整的AICSL系统部署说明，包括开发环境、生产环境配置和优化建议。
...
# 数据库
MONGODB_URI=mongodb://admin:admin123@mongodb:27017/aicsl
...
```bash
# MongoDB备份
docker-compose exec mongodb mongodump --db aicsl --out /backup/$(date +%Y%m%d_%H%M%S)

# 自动备份脚本
0 2 * * * docker-compose exec mongodb mongodump --db aicsl --out /backup/$(date +\%Y\%m\%d)
```

### 文件备份
```bash
# MinIO数据备份
docker-compose exec minio mc mirror /data/aicsl-files /backup/minio/$(date +%Y%m%d)
...
```bash
# 创建新版本
docker tag aicsl-backend:latest aicsl-backend:v2
...
```bash
# MongoDB恢复
docker-compose exec mongodb mongorestore /backup/20240101_020000/aicsl
